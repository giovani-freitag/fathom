import type { Dictionary } from './en.ts';

/**
 * The Brazilian Portuguese copy.
 */
export const PT_BR_DICTIONARY: Dictionary = {
    'chart.surface': 'Mapa de calor da liquidez do livro',

    'drawing.horizontalLine': 'Nível de preço',
    'drawing.trendLine': 'Linha de tendência',
    'drawing.remove': 'Remover a marca selecionada',
    'drawing.properties': 'Propriedades da marca',
    'drawing.label': 'Nome',
    'drawing.label.placeholder': 'Escreva na marca',
    'drawing.colour': 'Cor',
    'drawing.width': 'Espessura',
    'drawing.width.thin': 'Fina',
    'drawing.width.medium': 'Média',
    'drawing.width.thick': 'Grossa',
    'drawing.line': 'Traço',
    'drawing.line.solid': 'Contínuo',
    'drawing.line.dashed': 'Tracejado',
    'drawing.line.dotted': 'Pontilhado',
    'drawing.trail': 'Rastro',
    'drawing.trail.short': 'Curto',
    'drawing.trail.medium': 'Médio',
    'drawing.trail.long': 'Longo',
    'dock.label': 'Ferramentas do gráfico e de desenho',
    'dock.time': 'Tempo na tela',
    'drawing.undo': 'Desfazer',
    'drawing.redo': 'Refazer',
    'drawing.zone': 'Zona',
    'drawing.fibonacci': 'Retrações',
    'drawing.emoji': 'Emoji',
    'drawing.size': 'Tamanho',
    'drawing.size.thin': 'Pequeno',
    'drawing.size.medium': 'Médio',
    'drawing.size.thick': 'Grande',
    'drawing.tool.setup': 'Ajustar a ferramenta',
    'emoji.search': 'Buscar emoji',
    'emoji.search.empty': 'Nada responde por isso',
    'emoji.recent': 'Usados recentemente',
    'emoji.group.0': 'Carinhas',
    'emoji.group.1': 'Pessoas',
    'emoji.group.3': 'Natureza',
    'emoji.group.4': 'Comida',
    'emoji.group.5': 'Viagem',
    'emoji.group.6': 'Atividades',
    'emoji.group.7': 'Objetos',
    'emoji.group.8': 'Símbolos',
    'emoji.group.9': 'Bandeiras',
    'drawing.freehand': 'Caneta',
    'drawing.highlighter': 'Pincel de destaque',
    'drawing.laser': 'Laser',
    'drawing.measure': 'Medir',
    'drawing.select': 'Mover e selecionar',
    'drawing.keepTool': 'Manter a ferramenta após desenhar',

    'layer.depth': 'Book',
    'layer.depth.help': 'Tamanho em repouso em cada preço, como mudou ao longo do tempo',
    'layer.candles': 'Candles',
    'layer.candles.help': 'Onde o preço abriu, chegou e fechou em cada barra',
    'parameter.colourGain': 'Intensidade',
    'parameter.floorPercentile': 'Corte inferior',
    'parameter.saturationPercentile': 'Corte superior',
    'indicators.theChart': 'O gráfico',

    'indicator.sma': 'MMS',
    'indicator.sma.help': 'Média simples do fechamento da barra',
    'indicator.ema': 'MME',
    'indicator.ema.help': 'Média do fechamento da barra com peso no que é recente',
    'indicator.bollinger': 'Bollinger',
    'indicator.bollinger.help': 'Uma média com um canal na medida do quanto o preço tem se mexido',
    'indicator.bollinger.upper': 'Superior',
    'indicator.bollinger.lower': 'Inferior',
    'indicator.bollinger.middle': 'Média',
    'indicator.donchian': 'Donchian',
    'indicator.donchian.help': 'O maior e o menor preço das últimas barras',
    'indicator.donchian.upper': 'Máxima',
    'indicator.donchian.lower': 'Mínima',
    'indicator.donchian.middle': 'Meio',
    'indicator.rsi': 'IFR',
    'indicator.rsi.help': 'Quanto do movimento recente foi de alta, de zero a cem',
    'indicator.stochastic': 'Estocástico',
    'indicator.stochastic.help': 'Onde o fechamento está dentro da faixa que o preço percorreu',
    'indicator.stochastic.position': '%K',
    'indicator.stochastic.smoothed': '%D',
    'indicator.macd': 'MACD',
    'indicator.macd.help': 'A distância entre uma média rápida e uma lenta, e como ela muda',
    'indicator.macd.difference': 'Diferença',
    'indicator.macd.signal': 'Sinal',
    'indicator.macd.gap': 'Histograma',
    'indicator.atr': 'ATR',
    'indicator.supertrend': 'Supertrend',
    'indicator.supertrend.rising': 'Sob a alta',
    'indicator.supertrend.falling': 'Sobre a queda',
    'indicator.supertrend.help': 'Um stop mantido a um múltiplo da amplitude recente do preço',
    'indicator.psar': 'SAR parabólico',
    'indicator.psar.rising': 'Sob a alta',
    'indicator.psar.falling': 'Sobre a queda',
    'indicator.psar.help': 'Um stop que se aproxima mais rápido quanto mais o movimento dura',
    'indicator.adx': 'ADX',
    'indicator.adx.upward': 'Para cima',
    'indicator.adx.downward': 'Para baixo',
    'indicator.adx.help': 'Quão forte é a tendência, e qual lado está conduzindo',
    'indicator.mfi': 'Fluxo de dinheiro',
    'indicator.mfi.help': 'O saldo do movimento recente, ponderado pelo quanto foi negociado',
    'indicator.cci': 'CCI',
    'indicator.cci.help': 'O quanto o preço se afastou da sua média, em unidades do afastamento habitual',
    'indicator.cvd': 'Delta acumulado',
    'indicator.keltner': 'Canais de Keltner',
    'indicator.keltner.upper': 'Superior',
    'indicator.keltner.lower': 'Inferior',
    'indicator.atr.help': 'O quanto o preço tem andado por barra',
    'indicator.cvd.help': 'Tamanho líquido que cruzou o spread, somado',
    'indicator.delta': 'Delta',
    'indicator.delta.help': 'Tamanho líquido que cruzou o spread em cada barra',
    'indicator.pivots': 'Pontos de pivô',
    'indicator.pivots.help': 'Níveis que a sessão anterior fechou, mantidos ao longo desta',
    'indicator.pivots.central': 'Pivô',
    'indicator.pivots.r1': 'R1',
    'indicator.pivots.r2': 'R2',
    'indicator.pivots.r3': 'R3',
    'indicator.pivots.s1': 'S1',
    'indicator.pivots.s2': 'S2',
    'indicator.pivots.s3': 'S3',
    'parameter.pivotPeriod': 'Sessão',
    'pivotPeriod.daily': 'Diária',
    'pivotPeriod.weekly': 'Semanal',
    'parameter.pivotFormula': 'Espaçamento',
    'pivotFormula.classic': 'Clássico',
    'pivotFormula.fibonacci': 'Fibonacci',
    'indicator.keltner.help': 'Uma banda da largura do que o preço vem percorrendo',

    'indicator.vwap': 'VWAP',
    'indicator.vwap.help': 'O preço médio negociado até aqui, ponderado por quanto negociou em cada um',
    'parameter.vwapAnchor': 'Âncora',
    'vwapAnchor.session': 'Sessão',
    'vwapAnchor.window': 'Janela',
    'indicator.volume': 'Volume',
    'indicator.volume.help': 'Quanto mudou de mãos em cada barra, pelo lado que cruzou o spread',
    'indicator.volume.rising': 'Volume',
    'indicator.volume.falling': 'Volume',
    'indicator.volume.bought': 'Comprado',
    'indicator.volume.sold': 'Vendido',
    'parameter.showExecutions': 'Agressores',
    'parameter.showProfile': 'Perfil de volume',
    'parameter.showGaps': 'Lacunas de gravação',
    'parameter.showVolume': 'Volume negociado',
    'parameter.candleStyle': 'Estilo',
    'candleStyle.candles': 'Velas',
    'candleStyle.hollow': 'Velas vazadas',
    'candleStyle.bars': 'Barras',
    'candleStyle.line': 'Linha',
    'candleStyle.area': 'Área',
    'parameter.volumeMode': 'Volume mostra',
    'volumeMode.total': 'Total',
    'volumeMode.sides': 'Separado por lado',

    'parameter.periodBars': 'Barras',
    'parameter.deviations': 'Desvios',
    'parameter.fastBars': 'Rápida',
    'parameter.slowBars': 'Lenta',
    'parameter.signalBars': 'Sinal',
    'parameter.smoothingBars': 'Suavização',

    'parameter.source': 'Fonte',
    'source.close': 'Fechamento',
    'source.open': 'Abertura',
    'source.high': 'Máxima',
    'source.low': 'Mínima',
    'source.hl2': 'Meio da barra',
    'source.hlc3': 'Preço típico',
    'source.ohlc4': 'Média da barra',

    'editor.name': 'Como esta leitura se chama',
    'editor.untitled': 'Leitura sem nome',
    'editor.nameConnector': 'Como esta corretora se chama',
    'editor.untitledConnector': 'Corretora sem nome',
    'editor.shelfRefused': 'Este navegador recusou guardar. Nada foi salvo.',
    'editor.unsavedChoice': 'Ainda não salva',
    'editor.tooLarge': 'Esse arquivo é grande demais para ser uma leitura. Nada foi aberto.',
    'editor.notText': 'Esse arquivo não é texto. Nada foi aberto.',
    'editor.notBundle': 'Esse arquivo não é uma leitura exportada daqui. Nada foi aberto.',
    'editor.badPath': 'Esse pacote tem um arquivo que o Fathom não abre: {path}. Nada foi aberto.',
    'editor.unsaved': 'não salvo',
    'editor.replaced': '{name} saiu sem ser salva',
    'editor.closed': '{name} foi fechada, e continua na estante',
    'editor.resize': 'Redimensionar o editor. Arraste ou use as setas; Enter volta ao normal',
    'editor.wide': '{share} por cento da largura',
    'editor.tall': '{share} por cento da altura',
    'editor.save': 'Salvar',
    'editor.actions': 'O que dá para fazer com esta leitura',
    'editor.new': 'Começar uma nova',
    'editor.openSaved': 'Abrir salva',
    'editor.export': 'Exportar como arquivo',
    'editor.fault': 'Linha {line}: {message}',
    'editor.faults': 'O que está errado nesta leitura',
    'editor.help': 'Como escrever uma',
    'editor.import': 'Abrir um arquivo',
    'confirm.cancel': 'Manter',
    'editor.deleteTitle': 'Excluir esta leitura?',
    'editor.deleteBody': '“{name}” e o código dela vão embora de vez. O que estiver desenhado a partir dela vai junto.',
    'editor.deleteConfirm': 'Excluir',
    'editor.delete': 'Excluir esta leitura',
    'editor.deleteConnector': 'Excluir este conector',
    'editor.threw': 'Não desenhou nada e parou com: {message}',
    'budget.series': 'Isto desenha {drawn} séries, e uma leitura pode desenhar até {most}.',
    'budget.points': 'Isto desenha {drawn} pontos em uma série, e uma leitura pode desenhar até {most}.',
    'budget.uneven': 'Uma série tem {values} valores para {bars} barras.',
    'budget.band': 'Uma faixa sombreada nomeia uma série que esta leitura não desenha.',
    'indicators.shipped': 'Oficiais',
    'indicators.failed': 'Não desenhou nada: {message}',
    'indicators.missing': 'Uma leitura que esta versão não tem mais',
    'indicators.yours': 'Customs',
    'indicators.yoursEmpty': 'Nada escrito ainda.',
    'indicators.writeOne': 'Escrever uma leitura',
    'indicators.edit': 'Editar',
    'indicators.matchesYours': '{count} entre as que você escreveu',
    'indicators.matchesOurs': '{count} entre as da casa',
    'editor.open': 'Escrever uma leitura',
    'editor.title': 'Escrever uma leitura',
    'editor.titleConnector': 'Escrever um conector',
    'editor.code': 'O código desta leitura',
    'editor.codeConnector': 'O código deste conector',
    'editor.close': 'Fechar o editor',
    'editor.starting': 'Iniciando…',
    'editor.compilerLost': 'O compilador desta página parou de responder. Reabrir o editor costuma resolver.',
    'editor.drawing': 'Desenhando “{name}” no gráfico.',
    'editor.connected': '{name} adicionada. Escolha um par dela no seletor de contrato.',
    'editor.connectorDraft': 'Isto é um conector. Salve para adicionar a corretora.',
    'files.title': 'Os arquivos em que esta leitura foi escrita',
    'files.add': 'Adicionar um arquivo',
    'files.name': 'Como chamar o arquivo',
    'files.example': 'auxiliares.ts',
    'import.title': 'Trazer uma de fora',
    'import.body': 'De um repositório do GitHub ou de um pacote npm que contenha uma leitura escrita para este gráfico.',
    'import.where': 'O repositório ou pacote',
    'import.example': 'user/repo, ou gh/user/repo@main/pasta',
    'import.look': 'Ver',
    'import.files': 'O que seria aberto',
    'import.runsHere': 'Este código é de outra pessoa e roda nesta página assim que abrir. Traga apenas o que você mesmo rodaria.',
    'import.take': 'Abrir {count} arquivos',
    'files.rename': 'Toque duas vezes, ou F2, para renomear',
    'files.remove': 'Remover {path}',
    'files.entry': 'De onde o gráfico tira a leitura',
    'files.shown': 'O código em {path}',
    'files.added': '{path} adicionado',
    'files.removed': '{path} removido',
    'files.renamed': 'Renomeado para {path}',
    'console.title': 'Console',
    'console.clear': 'Limpar o console',
    'console.empty': 'Nada impresso ainda. Chame console.log na sua leitura para ver o que chega até ela.',
    'console.printed': 'Impresso',
    'console.warned': 'Aviso',
    'console.failed': 'Erro',
    'indicators.onTheChart': 'No gráfico',
    'indicators.add': 'Adicionar camada',
    'indicators.close': 'Fechar',
    'indicators.search': 'Buscar',
    'indicators.noMatch': 'Nada com esse nome',
    'indicators.colour': 'Cor',
    'indicators.removed': '{name} removido',
    'indicators.undo': 'Desfazer',
    'indicators.hideOne': 'Ocultar {layer}',
    'indicators.showOne': 'Mostrar {layer}',
    'indicators.mergeBand': 'Pôr na faixa acima',
    'indicators.splitBand': 'Dar faixa própria',
    'indicators.removeOne': 'Remover {layer}',
    'indicators.tuneOne': 'Ajustes de {layer}',
    'indicators.full': 'O gráfico já tem quantos consegue desenhar',
    'indicators.overPrice': 'Sobre o preço',
    'indicators.ownPane': 'Em faixa própria',
    'indicators.outOfReach': 'Precisa de {facts}, que esta corretora não publica',

    'venue.fact.book': 'tamanho em repouso por preço',
    'venue.fact.wholeBook': 'o livro inteiro, não os níveis mais próximos',
    'venue.fact.tape': 'o que de fato foi negociado',
    'venue.fact.volume': 'quanto foi negociado no candle',
    'venue.fact.takerSplit': 'volume separado pelo lado que atravessou',
    'venue.fact.tradeCount': 'quantas execuções formaram o candle',


    'markets.title': 'Contratos',
    'markets.yourTags': 'Suas tags',
    'markets.venues': 'Corretoras',
    'markets.sources': 'Tags e corretoras',
    'markets.tagsFor': 'Tags de {symbol}',
    'markets.allQuotes': 'Todas',
    'markets.shownOf': 'Mostrando {shown} de {matched} — refine a busca para ver o resto',
    'markets.halted': 'fora de negociação',
    'markets.recorded': 'gravado',
    'markets.openIt': 'Abrir {symbol} no gráfico',
    'markets.tagsColumn': 'Etiquetas',
    'markets.pairColumn': 'Par',
    'markets.baseQuoteColumn': 'Base e cotação',
    'markets.venueColumn': 'Corretora',
    'markets.noteColumn': 'O que é',
    'markets.favourites': 'Favoritos',
    'markets.newTag': 'Nova tag',
    'markets.editTag': 'Editar esta tag',
    'markets.editConnector': 'Editar este conector',
    'markets.giveUp': 'Cancelar',
    'markets.makeTag': 'Criar a tag',
    'markets.saveTag': 'Salvar',
    'markets.tagLabel': 'Nome da tag',
    'markets.removeTag': 'Excluir esta tag',
    'markets.removeTagTitle': 'Excluir esta tag?',
    'markets.removeTagBody': 'A {tag} guarda {count}. Excluir leva eles junto, e não dá para desfazer — os pares em si continuam nas corretoras.',
    'markets.onePair': 'um par',
    'markets.somePairs': '{count} pares',
    'markets.removeTagConfirm': 'Excluir',
    'markets.tagColour': 'Cor',
    'markets.anyColour': 'Qualquer outra cor',
    'markets.removeVenue': 'Remover esta corretora',
    'markets.emptyTag': 'Nada nesta tag ainda. Escolha uma corretora e marque os pares que quiser guardar.',
    'markets.addVenue': 'Adicionar corretora',
    'markets.searchPairs': 'Buscar pares',
    'markets.noPairs': 'Nada com esse nome nesta corretora',
    'markets.reading': 'Perguntando à corretora o que ela negocia…',
    'markets.readingBeforeSearch': 'Perguntando à corretora o que ela negocia — sua busca roda no que ela responder.',
    'markets.readingOf': 'Ainda lendo — {read} de {total} até aqui',
    'markets.readingSome': 'Ainda lendo — {read} até aqui',
    'markets.askingVenue': 'Perguntando à corretora sobre o que você digitou…',
    'markets.retry': 'Tentar de novo',
    'markets.noConnector': 'Nada nesta build sabe ler esta corretora.',
    'markets.nothingToDraw': 'Esta corretora declara não ter livro, fita nem candles, então não há o que desenhar',
    'markets.addTo': 'Guardar {symbol} em {tag}',
    'markets.removeFrom': 'Tirar {symbol} de {tag}',


    'coverage.perColumn': '/col',
    'coverage.gapTitle': 'Trechos sem gravação nesta janela',

    'legend.book': 'book',
    'legend.notRecorded': 'livro não gravado aqui',

    'interval.auto': 'Auto · {interval}',
    'interval.label': 'Intervalo da barra',
    'readout.open': 'A',
    'readout.high': 'M',
    'readout.low': 'm',
    'readout.close': 'F',
    'readout.bid': 'COMPRA {size} {asset} em {price}',
    'readout.ask': 'VENDA {size} {asset} em {price}',
    'readout.empty': 'nada parado em {price}',
    'readout.barWhole': 'barra de {interval}, gravada por inteiro',
    'readout.barPartial': 'barra de {interval}, {frames} de {expected} segundos gravados',
    'readout.barForming': 'barra de {interval}, ainda sendo escrita',
    'readout.fromMid': '{delta} · {percent} do meio',
    'readout.buy': 'compra {size}',
    'readout.sell': 'venda {size}',
    'readout.traded': 'negociado {sides}',
    'readout.tradeCount': '{count}x · maior {size}',

    'unit.seconds': '{value}s',
    'unit.minutes': '{value}min',
    'unit.hours': '{value}h',
    'unit.days': '{value}d',

    'span.1m': '1m',
    'span.5m': '5m',
    'span.15m': '15m',
    'span.1h': '1h',
    'span.4h': '4h',
    'span.1d': '1d',
    'span.3d': '3d',
    'span.1w': '1sem',

    'span.label': 'Janela de tempo',

    'page.returnToLive': 'Voltar ao tempo real',
    'page.probing': 'Carregando a gravação…',
    'page.empty': 'Nada gravado ainda. A gravação precisa estar ligada: um livro de ofertas não pode ser recuperado depois.',
    'page.retry': 'Tentar de novo',

    'settings.open': 'Preferências',
    'settings.title': 'Preferências',
    'settings.close': 'Fechar',

    'settings.appearance': 'Aparência',
    'settings.language': 'Idioma',
    'settings.theme': 'Tema',
    'settings.grid': 'Grade',
    'settings.grid.none': 'Nenhuma',
    'settings.grid.price': 'Preço',
    'settings.grid.both': 'Preço e tempo',
    'theme.system': 'Do sistema',
    'theme.light': 'Claro',
    'theme.dark': 'Escuro',
    'colour.green': 'Verde',
    'colour.red': 'Vermelho',
    'colour.amber': 'Âmbar',
    'colour.teal': 'Turquesa',
    'colour.violet': 'Violeta',
    'colour.blue': 'Azul',
    'colour.white': 'Branco',
    'colour.grey': 'Cinza',
    'language.en': 'English',
    'language.pt-BR': 'Português',

    'settings.drawn': 'O que está desenhado',
    'settings.recordedSoFar': 'Gravado até agora',
    'settings.resolution': 'Resolução',
    'settings.perColumn': '{value} por coluna',
    'settings.priceBand': 'Faixa de preço',
    'settings.perRow': '{value} por linha',
    'settings.columnsLoaded': 'Colunas carregadas',
    'settings.gapsInWindow': 'Lacunas na janela',
    'about.whatsNew': 'Novidades',
    'about.guide': 'Ler o guia',
    'about.releasedOn': 'Publicado em {date}',
    'about.unreleased': 'Compilado a partir da árvore de trabalho',

    'settings.recordingIsGlobal': 'O coletor roda esteja o livro desenhado ou não. Ocultar o livro para de desenhá-lo e nada mais.',
    'settings.backfillNote': 'Candles e volume vêm da corretora, então o gráfico já abre com histórico. O livro não: ele só cobre o período em que a gravação esteve ligada, e um livro de ofertas não pode ser recuperado depois.',

    'recording.reading': 'Lendo o que está sendo gravado…',
    'recording.readFailed': 'Não foi possível ler o que está sendo gravado. Tentar de novo',
    'recording.title': 'Gravação',
    'recording.usage': '{used} de {total}',
    'recording.contractsHelp': 'O que é capturado é o livro e tudo que negocia nele, na grade de preço mostrada. Desligar um contrato interrompe a gravação dele; tudo o que ele já capturou permanece.',
    'recording.toggle': 'Gravar {symbol}',
    'recording.saveFailed': 'Não foi possível salvar essa alteração.',
    'recording.addPair': 'Escolher o que gravar',
    'recording.startThis': 'Gravar {symbol}',
    'recording.stopThis': 'Parar de gravar {symbol}',
    'recording.resumeThis': 'Gravar {symbol} de novo',
    'recording.gridFor': 'Preço por linha de {symbol}',
    'recording.noGridYet': 'Nenhum preço chegou ainda, então não há escada para escolher.',
    'recording.onVenues': 'Gravando {pairs} em {venues}',
    'recording.someOff': '{count} desligados',
    'recording.noneYet': 'Nada está sendo gravado ainda.',
    'recording.pickerTitle': 'Gravar um par',
    'recording.venuesWithBook': 'Corretoras com livro',
    'recording.gridPrompt': 'Cada linha do mapa de calor cobre',
    'recording.regrid': 'Mudar a grade em que {symbol} grava — agora {grid}',
    'recording.remove': 'Excluir {symbol} e tudo que ele gravou',
    'recording.removeTitle': 'Excluir esta gravação?',
    'recording.removeBody': 'Tudo que foi gravado de {symbol} é apagado, e o espaço volta. Um livro de ofertas não pode ser gravado de novo, então o que sai daqui não volta. Desligar em vez disso para a gravação e mantém tudo.',
    'recording.removeBodyOff': 'Tudo que foi gravado de {symbol} é apagado, e o espaço que ocupava volta. Um livro de ofertas não pode ser gravado de novo, então o que vai daqui foi. Ele já está desligado, então deixá-lo em paz não custa nada além do espaço.',
    'recording.removeConfirm': 'Excluir',
    'recording.regridCost': 'O que já foi gravado fica na grade em que foi escrito. Uma janela que alcança antes da mudança é lida na mais grossa das duas, e uma grade mais fina não mostra detalhe que o trecho antigo nunca teve.',
    'recording.venuesWithoutBook': 'Nenhum conector daqui lê livro de {venues}, então ainda não há o que gravar nelas.',
    'recording.recordingHere': 'Gravando',
    'recording.everythingElse': 'Todo o resto',
    'recording.switchedOffHere': 'Desligados',
    'recording.noGrid': 'sem tick publicado',
    'recording.choose': 'escolher a grade',
    'recording.ceiling': 'Teto de armazenamento',
    'recording.ceilingHelp': 'Ao atingir o teto, os dias mais antigos são descartados para abrir espaço para os mais novos.',

    'demo.connecting': 'Conectando à corretora e espelhando o livro de ofertas. As primeiras colunas aparecem em segundos.',
    'demo.stopped': 'Gravação interrompida. Recarregue para começar de novo.',
    'demo.wasHidden': 'Esta aba ficou em segundo plano, onde a gravação desacelera. Esses segundos aparecem como lacunas em vez de dados inventados.',
    'demo.dismiss': 'Dispensar',
    'demo.refusedTitle': 'Este navegador não deixa a demonstração gravar',
    'demo.refusedBody': 'A página guarda o que grava dentro do seu navegador. Janelas anônimas e algumas configurações de privacidade bloqueiam isso, e uma gravação que só existe enquanto você assiste não tem outro lugar para ficar.',

    'alert.dismiss': 'Dispensar',
    'failure.silent': 'O servidor não respondeu. Verifique se ele está rodando.',
    'failure.server': 'O servidor não conseguiu responder. Ele pode não estar conseguindo alcançar a gravação.',
    'failure.refused': 'O servidor recusou a solicitação.',
    'failure.generic': 'Não foi possível carregar esta janela de tempo.',
};
