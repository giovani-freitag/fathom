...
/** One pair a search found, and the venue it was found on. */
...
/**
 * What a reader typed, matched against every listing already read.
 *
 * Search stops being a question about one venue. A reader who knows the ticker
 * knows the ticker; which catalogue it happens to sit in is something the row
 * can say for itself, and making them pick the catalogue first is asking them
 * to answer a question in order to ask their own.
 *
 * Only what has been read: a venue nobody has opened has no listing in memory,
 * and fetching six of nine hundred pairs to answer three keystrokes is a
 * question the reader did not ask either. What was covered is said underneath.
 *
 * @param listings - Every venue's instruments, by venue, as far as they are read.
 * @param filter - What was typed and which quote was picked.
 * @returns The best matches across all of them, and how many matched in all.
 */
export function searchAcross(
    listings: Readonly<Record<string, readonly VenueInstrument[]>>,
    filter: PairFilter,
): { readonly shown: readonly FoundPair[]; readonly matched: number } {
    const found: FoundPair[] = [];
    let matched = 0;

    for (const [venue, instruments] of Object.entries(listings)) {
        const narrowed = narrowPairs(instruments, filter);
        matched += narrowed.matched;
        for (const instrument of narrowed.shown) {
            found.push({ venue, instrument });
        }
    }

    // Ranked again across the venues, because each was ranked only within its
    // own: an exact match on the second venue read belongs above a prefix match
    // on the first.
    const wanted = filter.query.trim().toUpperCase();
    if (wanted !== '') {
        found.sort((one, other) => rankAgainst(one.instrument, wanted) - rankAgainst(other.instrument, wanted)
            || one.instrument.symbol.localeCompare(other.instrument.symbol)
            || one.venue.localeCompare(other.venue));
    }

    return { shown: found.slice(0, ROWS_SHOWN), matched };
}
